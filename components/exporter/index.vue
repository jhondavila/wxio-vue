<template>
  <div class="exporter-wrapper">
    <slot>
      <el-button :id="idName" type="info" @click="generate"></el-button>
    </slot>
  </div>
</template>
...
export default {
  ...
  methods: {
    getProcessedJsonData(data, fields) {
      return data.map(i => {
        let obj = {};
        for (let p in fields) {
          obj[fields[p]] = i[p];
    ...
    async generate(data) {
      ...
      if (this.type === "json") {
        let json = this.getProcessedJsonData(data, this.downloadFields);
        return this.export(
          JSON.stringify(json, null, "\t"),
          this.name.replace(".xls", ".json"),
          "text/json"
        );
      } else {
        let json = this.getProcessedJsonColumn(data, this.downloadFields);
        if (this.type === "html") {
    ...
    /*
		getProcessedJsonColumn
		---------------
		Get only the data to export, if no fields are set return all the data
		*/
    getProcessedJsonColumn(data, header) {
