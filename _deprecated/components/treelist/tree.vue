...
export default {
  ...
  methods: {
    ...
    setCheckedNodes(values, key, data) {
      // debugger
      key = key || "id";
      data = data || this.data;
      for (const node of data) {
        if (values.findIndex(val => val === node[key]) > -1) {
          this.$set(node, "checked", true);
        } else {
          this.$set(node, "checked", false);
        }
        if (node.children && node.children.length) {
          this.setCheckedNodes(values, key, node.children);
        }
      }
    },
    eachNodes(fn, data) {
      ...
      for (const node of data) {
        fn(node);
        if (node.children && node.children.length) {
          this.eachNodes(fn, node.children);
          // this.setCheckedNodes(values, key, node.children);
        }
      }
    }
